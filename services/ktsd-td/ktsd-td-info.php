...
$querry_info_ctkt = 'SELECT ctktsd.*, doituong_ktsd."tenDoiTuongKTSD",
                        ...
                        thongtincptd."mucDichTD", thongtincptd."quyMoTD",
                        thongtincptd."tinhTrangGiayPhep" FROM "CT_KTSD" ctktsd
                        ...
                        LEFT JOIN "ThongTinCP_TD" thongtincptd ON ctktsd.id = thongtincptd.ma_congtrinhktsd
...
/*---- Thông tin điểm KTSD td ----*/
$querry_poi_ktsd_td = 'SELECT "DiemTD_NDD".*, tangchuanuoc.name "TangChuaNuoc" FROM "DiemTD_NDD" 
                                LEFT JOIN "CT_KTSD" ctktsd on "DiemTD_NDD".ma_congtrinhktsd = ctktsd.id
                                LEFT JOIN "TangChuaNuoc" tangchuanuoc on "DiemTD_NDD".ma_tangchuanuoc = tangchuanuoc.id
                                WHERE ma_congtrinhktsd =' . "'" . $macongtrinh . "'";
$result_poi_ktsd_td = pg_query($tiengiang_db, $querry_poi_ktsd_td);
if (!$result_poi_ktsd_td) {
    echo "Không có dữ liệu.\n";
    exit;
}
$data_poi_ktsd_td = array();
while ($row = pg_fetch_assoc($result_poi_ktsd_td)) {
    $data_poi_ktsd_td[] = $row;
}
$jsonData_poi_ktsd_td = json_encode($data_poi_ktsd_td);
$original_poi_ktsd_td = json_decode($jsonData_poi_ktsd_td, true);
$option_poi_ktsd_td = array();
foreach ($original_poi_ktsd_td as $key => $value_poi_ktsd_td) {
    $option_poi_ktsd_td[] = array(
        'STT' => $key + 1,
        "id_gieng" => $value_poi_ktsd_td['id'],
        "sohieugieng" => $value_poi_ktsd_td['soHieuGiengTD'],
        "tangchuanuoc" => $value_poi_ktsd_td['TangChuaNuoc'],
        "toadoX" => (float)$value_poi_ktsd_td['toaDoX'],
        "toadoY" => (float)$value_poi_ktsd_td['toaDoY'],
        "chieuSauDuKienTD" => $value_poi_ktsd_td['chieuSauDuKienTD'],
        "soLuongGiengTD" => $value_poi_ktsd_td['soLuongGiengTD'],
        "tinhtrangthamdo" => $value_poi_ktsd_td['tinhtrangthamdo'] == 'f' ? 'Đã trám lấp' : 'Đang hoạt động',
        "vungbaoho" => (float)$value_poi_ktsd_td['vungBHVS'],
        "motaVungbaoho" => $value_poi_ktsd_td["moTaVungBHVS"]
    );
}
$final_poi_ktsd_td = json_encode($option_poi_ktsd_td);

/*---- Thông tin giấy phép thăm dò ----*/
$querry_gp_td = 'SELECT thongtincp_td.*, enterprise.name "TenDoanhNghiep", donvicapphep.name "DonViCapPhep", 
                        ctktsd."coSoKTSD", loaigiayphep.name "LoaiGiayPhep" FROM "ThongTinCP_TD" thongtincp_td
                        LEFT JOIN "CT_KTSD" ctktsd ON thongtincp_td.ma_congtrinhktsd = ctktsd.id
                        LEFT JOIN "Enterprise" enterprise on thongtincp_td.ma_doanhnghiep = enterprise.id
                        LEFT JOIN "DonViCapPhep" donvicapphep on thongtincp_td.ma_donvicapphep = donvicapphep.id
                        LEFT JOIN "LoaiGiayPhep" loaigiayphep on thongtincp_td.ma_loaigiayphep = loaigiayphep.id
                        WHERE ma_congtrinhktsd =' . "'" . $macongtrinh . "'";

$result_gp_td = pg_query($tiengiang_db, $querry_gp_td);
if (!$result_gp_td) {
    echo "Không có dữ liệu.\n";
    exit;
}
$data_gp_td = array();
while ($row = pg_fetch_assoc($result_gp_td)) {
    $data_gp_td[] = $row;
}
$jsonData_gp_td = json_encode($data_gp_td);
$original_gp_td = json_decode($jsonData_gp_td, true);
$option_gp_td = array();
foreach ($original_gp_td as $key => $value_gp_td) {
    if ($value_gp_td["ngayCapPhep"] != null) {
        $ngaycapphep = explode("-", $value_gp_td["ngayCapPhep"]);
        $ngaycapphep_new = $ngaycapphep[2] . "/" . $ngaycapphep[1] . "/" . $ngaycapphep[0];
    } else {
        $ngaycapphep_new = '';
    }

    if ($value_gp_td["ngayHetHan"] != null) {
        $ngayhethan = explode("-", $value_gp_td["ngayHetHan"]);
    ...
    /*** Kiểm tra giấy phép ***/
    if (isset(explode("/", $value_gp_td['taiLieuDinhKem'])[9])) {
        $filename = explode("/", $value_gp_td['taiLieuDinhKem'])[9];
    } else {
        $filename = explode("/", $value_gp_td['taiLieuDinhKem'])[6];
    }

    $option_gp_td[] = array(
        'STT' => $key + 1,
        "id_gp" => $value_gp_td['id'],
        "sogiayphep" => $value_gp_td['soGiayPhepTD'],
        "loaigiayphep" => $value_gp_td['LoaiGiayPhep'],
        "donvicapphep" => $value_gp_td['DonViCapPhep'],
        "doanhnghiep" => $value_gp_td['TenDoanhNghiep'],
        "coso_sanxuat" => $value_gp_td['coSoKTSD'],
        "ngaycapphep" => $ngaycapphep_new,
        "ngayhethan" => $ngayhethan_new,
        "thoihan" => $value_gp_td['thoiHanGiayPhep'],
        "hieuluc" => $value_gp_td['tinhTrangGiayPhep'] == 't' ? "Còn hiệu lực" : "Hết hiệu lực",
        "banso_giayphep" =>  $filename,
        "link_giayphep" => $value_gp_td['taiLieuDinhKem']
    );
}
$final_gp_td = json_encode($option_gp_td);
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <form class="form-horizontal" method="post" id="info_ctkt_form" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group row list-ctkt">
                                                <label style="margin-top: 7px" class="col-md-2 control-label">
                                                    <b>Mục đích thăm dò</b>
                                                </label>
                                                <div class="col-md-4">
                                                    <?php
                                                    echo '<input class="form-control input-sm" 
                                                        type="text" name="mucdich_td" id="mucdich_td"' .
                                                        'value=' . '"' . $data_info_ctkt[$i_check]["mucDichTD"] . '"' . '>'
                                                    ?>
                                                </div>
                                                <label style="margin-top: 7px" class="col-md-2 control-label">
                                                    <b>Quy mô thăm dò</b>
                                                ...
                                                <div class="col-md-4">
                                                    <?php
                                                    echo '<input class="form-control input-sm" 
                                                        type="text" name="quymo_td" id="quymo_td"' .
                                                        'value=' . '"' . $data_info_ctkt[$i_check]["quyMoTD"] . '"' . '>'
                                                    ?>
                                                </div>
                                            </div>
                                            <div class="form-group row list-ctkt">
                ...
                <div class="main-content">
                    <div class="container-fluid">
                        <div class="page-title" style="text-align: center">
                            <h3>
                                <b>CÔNG TRÌNH THĂM DÒ NƯỚC DƯỚI ĐẤT</b>
                            </h3>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                            ...
                                            <div class="col-md-12">
                                                <form class="form-horizontal">
                                                    ...
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            echo '<input class="form-control input-sm" id="mucdichtd"
                                                                type="text"' . 'value=' . '"' . $data_info_ctkt[$i_check]["mucDichTD"] . '"' . '>'
                                                            ?>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            echo '<input class="form-control input-sm" id="quymotd"
                                                                type="text"' . 'value=' . '"' . $data_info_ctkt[$i_check]["quyMoTD"] . '"' . '>'
                                                            ?>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Thời hạn khai thác sử dụng</b>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        <h4 class="card-title text-bold text-info">
                                            Thông tin điểm Thăm dò Nước dưới đất
                                        </h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="poi_ktsd_td" class="table table-sm table-hover table-bordered">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_poi_ktsd_td">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_poi_ktsd_td">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_poi_ktsd_td">
                                                        <i class="ti-export pdd-right-5"></i>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="gp_td" class="table table-sm table-hover table-bordered">
                                                    <!-- DOM after Process Data -->
                                                </table>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_gp_td">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_gp_td">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_gp_td">
                                                        <i class="ti-export pdd-right-5"></i>
                                                        <span>Xuất dữ liệu</span>
                                                    </a> -->
                                                </div>
    ...
    <script>
        ...
        var columns_title = [{
            ...
            },
            {
                ...
                cellClick: function(e, row, formatterParams) {
                    ...
                    window.open("../../map.php?idpoi=" + idgieng + "&type=td")
                }
            },
            {
                title: "#",
            ...
            },
            {
                ...
                formatterParams: {
                    ...
                    <?php
                    echo 'urlPrefix: "form-poi-ktsd-td.php?macongtrinh=' . $macongtrinh . '&idgieng=",'
                    ?>
                    target: "_blank",
                }
            },
            ...
            {
                title: "Chiều sâu dự kiến",
                field: "chieuSauDuKienTD",
                hozAlign: "center"
            },
            {
                title: "Số lượng giếng thăm dò",
                field: "soLuongGiengTD",
                hozAlign: "center"
            },
            {
                title: "Tình trạng thăm dò",
                field: "tinhtrangthamdo",
                hozAlign: "center"
            },
            {
                title: "Vùng bảo hộ",
        ...
        /*---- Lấy JSON bằng Php ----*/
        <?php
        $script = 'var tabledata = ' . $final_poi_ktsd_td . ";";
        echo $script;
        ?>

        $("#poi_ktsd_td").css("border-top", "1px solid #dee2e6");
        var selected_id;
        var table = new Tabulator("#poi_ktsd_td", {
        ...
        /*** Delete DiemKTSD_td ***/
        $("#delete_poi_ktsd_td").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                ...
                if (confirm("Bạn chắc chắn muốn xóa các điểm khai thác đã chọn này ?")) {
                    ...
                    var response_post = $.post("delete-poi-ktsd-td.php", {
                        <?php
        ...
        /*** Create new DiemKTSD_td (Button) ***/
        $("#create_poi_ktsd_td").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-poi-ktsd-td.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        ...
        var columns_title = [{
            ...
            },
            {
                ...
                formatterParams: {
                    ...
                    <?php
                    echo 'urlPrefix: "form-gp-td.php?macongtrinh=' . $macongtrinh . '&idgp=",'
                    ?>
                    target: "_blank",
                }
            },
        ...
        /*---- Lấy JSON bằng Php ----*/
        <?php
        $script = 'var tabledata = ' . $final_gp_td . ";";
        echo $script;
        ?>
        $("#gp_td").css("border-top", "1px solid #dee2e6");
        var selected_id;
        var table = new Tabulator("#gp_td", {
            columns: columns_title,
            rowSelectionChanged: function(data, rows) {
                selected_id = data;
            }
        ...
        /*** Delete ThongtinCP_td ***/
        $("#delete_gp_td").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                ...
                if (confirm("Bạn chắc chắn muốn xóa các giấy phép đã chọn này ?")) {
                    ...
                    var response_post = $.post("delete-gp-td.php", {
                        <?php
                        echo '"macongtrinh":' . $macongtrinh . ",";
                        ?> "id_gp": id_gp
                    }).done(function(data) {
        ...
        /*** Create new ThongtinCP_td (Button) ***/
        $("#create_gp_td").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-gp-td.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        ...
        $("#update_info_ctkt").click(function() {
            ...
            /*** ThongtinCP_td ***/
            var mucdich_td = $("#mucdich_td").val();
            var quymo_td = $("#quymo_td").val();

            var response_post = $.post("update-td-info.php", {
                <?php
                echo '"macongtrinh":' . $macongtrinh . ",";
                ?>
                /*** CT_KTSD ***/
                ...
                if (data != "error") {
                    ...
                    $("#mucdichtd").val(mucdich_td);
                    $("#quymotd").val(quymo_td);
                } else {
