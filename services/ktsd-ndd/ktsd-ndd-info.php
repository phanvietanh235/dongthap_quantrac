...
$querry_info_ctkt = 'SELECT ctktsd.*, doituong_ktsd."tenDoiTuongKTSD",
                        ...
                        thongtincp_ndd."tongSoGiengKT", thongtincp_ndd."tongLuuLuongKT",
                        thongtincp_ndd."phamViCapNuoc", thongtincp_ndd."tinhTrangGiayPhep" FROM "CT_KTSD" ctktsd
                        ...
                        LEFT JOIN "ThongTinCP_NDD" thongtincp_ndd ON ctktsd.id = thongtincp_ndd.ma_congtrinhktsd
...
$querry_poi_ktsd_ndd = 'SELECT "DiemKTSD_NDD".*, tangchuanuoc.name "TangChuaNuoc" FROM "DiemKTSD_NDD" 
                            LEFT JOIN "CT_KTSD" ctktsd on "DiemKTSD_NDD".ma_congtrinhktsd = ctktsd.id
                            LEFT JOIN "TangChuaNuoc" tangchuanuoc on "DiemKTSD_NDD".ma_tangchuanuoc = tangchuanuoc.id
                            WHERE ma_congtrinhktsd =' . "'" . $macongtrinh . "'";
$result_poi_ktsd_ndd = pg_query($tiengiang_db, $querry_poi_ktsd_ndd);
if (!$result_poi_ktsd_ndd) {
    echo "Không có dữ liệu.\n";
    exit;
}
$data_poi_ktsd_ndd = array();
while ($row = pg_fetch_assoc($result_poi_ktsd_ndd)) {
    $data_poi_ktsd_ndd[] = $row;
}
$jsonData_poi_ktsd_ndd = json_encode($data_poi_ktsd_ndd);
$original_poi_ktsd_ndd = json_decode($jsonData_poi_ktsd_ndd, true);
$option_poi_ktsd_ndd = array();
foreach ($original_poi_ktsd_ndd as $key => $value_poi_ktsd_ndd) {
    $option_poi_ktsd_ndd[] = array(
        'STT' => $key + 1,
        "id_gieng" => $value_poi_ktsd_ndd['id'],
        "sohieugieng" => $value_poi_ktsd_ndd['soHieuGieng'],
        "tangchuanuoc" => $value_poi_ktsd_ndd['TangChuaNuoc'],
        "toadoX" => (float)$value_poi_ktsd_ndd['toaDoX'],
        "toadoY" => (float)$value_poi_ktsd_ndd['toaDoY'],
        "chieuSau" => $value_poi_ktsd_ndd['chieuSauGiengTu'] . " - " . $value_poi_ktsd_ndd['chieuSauGiengDen'],
        "mucNuocTinh" => $value_poi_ktsd_ndd['chieuSauMNTinh'],
        "mucNuocDong" => $value_poi_ktsd_ndd['chieuSauMNDongLonNhat'],
        "chedoKT" => $value_poi_ktsd_ndd['cheDoKhaiThac'],
        "luuluongKT" => $value_poi_ktsd_ndd['luuLuongKTCP'],
        "tinhtranggieng" => $value_poi_ktsd_ndd['tinhTrangGieng'] == 'f' ? 'Đã trám lấp' : 'Đang hoạt động',
        "vungbaoho" => (float)$value_poi_ktsd_ndd['vungBHVS'],
        "motaVungbaoho" => $value_poi_ktsd_ndd["moTaVungBHVS"]
    );
}
$final_poi_ktsd_ndd = json_encode($option_poi_ktsd_ndd);

/*---- Thông tin giấy phép khai thác ----*/
$querry_gp_ndd = 'SELECT thongtincp_ndd.*, enterprise.name "TenDoanhNghiep", donvicapphep.name "DonViCapPhep", 
                    ctktsd."coSoKTSD", loaigiayphep.name "LoaiGiayPhep" FROM "ThongTinCP_NDD" thongtincp_ndd
...
$jsonData_gp_ndd = json_encode($data_gp_ndd);
$original_gp_ndd = json_decode($jsonData_gp_ndd, true);
$option_gp_ndd = array();
foreach ($original_gp_ndd as $key => $value_gp_ndd) {
    if ($value_gp_ndd["ngayCapPhep"] != null) {
        $ngaycapphep = explode("-", $value_gp_ndd["ngayCapPhep"]);
        $ngaycapphep_new = $ngaycapphep[2] . "/" . $ngaycapphep[1] . "/" . $ngaycapphep[0];
    } else {
        $ngaycapphep_new = '';
    }

    if ($value_gp_ndd["ngayHetHan"] != null) {
        $ngayhethan = explode("-", $value_gp_ndd["ngayHetHan"]);
    ...
    /*** Kiểm tra giấy phép ***/
    if (isset(explode("/", $value_gp_ndd['taiLieuDinhKem'])[9])) {
        $filename = explode("/", $value_gp_ndd['taiLieuDinhKem'])[9];
    } else {
        $filename = explode("/", $value_gp_ndd['taiLieuDinhKem'])[6];
    }

    $option_gp_ndd[] = array(
        'STT' => $key + 1,
        "id_gp" => $value_gp_ndd['id'],
        "sogiayphep" => $value_gp_ndd['soGiayPhepNDD'],
        "loaigiayphep" => $value_gp_ndd['LoaiGiayPhep'],
        "donvicapphep" => $value_gp_ndd['DonViCapPhep'],
        "doanhnghiep" => $value_gp_ndd['TenDoanhNghiep'],
        "coso_sanxuat" => $value_gp_ndd['coSoKTSD'],
        "ngaycapphep" => $ngaycapphep_new,
        "ngayhethan" => $ngayhethan_new,
        "thoihan" => $value_gp_ndd['thoiHanGiayPhep'],
        "hieuluc" => $value_gp_ndd['tinhTrangGiayPhep'] == 't' ? "Còn hiệu lực" : "Hết hiệu lực",
        "banso_giayphep" =>  $filename,
        "link_giayphep" => $value_gp_ndd['taiLieuDinhKem']
    );
}
$final_gp_ndd = json_encode($option_gp_ndd);

/*---- Thông tin tiền cấp quyền ----*/
$querry_tiencq_ndd = 'SELECT tiencq_ndd.*, 
                          thongtincp_ndd."soGiayPhepNDD", 
                          thongtincp_ndd."tinhTrangGiayPhep" FROM "TienCapQuyen_NDD" tiencq_ndd
                          LEFT JOIN "ThongTinCP_NDD" thongtincp_ndd ON tiencq_ndd.ma_giayphepndd = thongtincp_ndd.id
                          LEFT JOIN "CT_KTSD" ctktsd ON thongtincp_ndd.ma_congtrinhktsd = ctktsd.id
                          WHERE ma_congtrinhktsd =' . "'" . $macongtrinh . "'";
$result_tiencq_ndd = pg_query($tiengiang_db, $querry_tiencq_ndd);
if (!$result_tiencq_ndd) {
    echo "Không có dữ liệu.\n";
    exit;
}
$data_tiencq_ndd = array();
while ($row = pg_fetch_assoc($result_tiencq_ndd)) {
    $data_tiencq_ndd[] = $row;
}

$jsonData_tiencq_ndd = json_encode($data_tiencq_ndd);
$original_tiencq_ndd = json_decode($jsonData_tiencq_ndd, true);
$option_tiencq_ndd = array();

if (count($data_tiencq_ndd) != 0) {
    foreach ($original_tiencq_ndd as $key => $value_tiencq_ndd) {
        if ($value_tiencq_ndd["ngayBanHanhQDPD"] != null) {
            $ngayBanHanhQDPD = explode("-", $value_tiencq_ndd["ngayBanHanhQDPD"]);
        ...
        $option_tiencq_ndd[] = array(
            'STT' => $key + 1,
            "id_tiencq" => $value_tiencq_ndd['id'],
            "sogiayphep" => $value_tiencq_ndd['soGiayPhepNDD'],
            "hieuluc" => $value_tiencq_ndd['tinhTrangGiayPhep'] == 't' ? "Còn hiệu lực" : "Hết hiệu lực",
            "so_quyetdinh_pheduyet" => $value_tiencq_ndd['quyetDinhPheDuyetTCQ'],
            "ngay_banhanh_quyetdinh" => $ngayBanHanhQDPD_new,
            "tongtien_nop" => $value_tiencq_ndd['tongTienNop'],
            "tien_hangnam" => nested_tienhangnam($value_tiencq_ndd['soTienNopHangNam'], $value_tiencq_ndd['tongTienNop']),
            "phuongan_nop" => $value_tiencq_ndd['phuongAnNop'],
            "tailieu_dinhkem" => '',
        );
    }
    $final_tiencq_ndd = json_encode($option_tiencq_ndd);
} else {
    $final_tiencq_ndd = json_encode([]);
}
...
function nested_tienhangnam($tien_hangnam, $tongtien)
{
    ...
    $tien_hangnam_arr[] = array(
        "id" => "Còn lại",
        "sotien_hangnam" => (int)$tongtien - $tongtien_ky < 0 ? 0 : (int)$tongtien - $tongtien_ky,
    );
    return $tien_hangnam_arr;
}

/*** Thông tin tình hình vi phạm ***/
if (isset($data_gp_ndd[0]["ma_doanhnghiep"])) {
    $querry_thvp_ndd = 'SELECT thvp_ndd.*, enterprise.name "TenDoanhNghiep" FROM "TinhHinhViPham_NDD" thvp_ndd
                        LEFT JOIN "Enterprise" enterprise on thvp_ndd.ma_doanhnghiep = enterprise.id
                        WHERE ma_doanhnghiep =' . "'" . $data_gp_ndd[0]["ma_doanhnghiep"] . "'";
    $result_thvp_ndd = pg_query($tiengiang_db, $querry_thvp_ndd);
    if (!$result_thvp_ndd) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_thvp_ndd = array();
    while ($row = pg_fetch_assoc($result_thvp_ndd)) {
        $data_thvp_ndd[] = $row;
    }

    $jsonData_thvp_ndd = json_encode($data_thvp_ndd);
    $original_thvp_ndd = json_decode($jsonData_thvp_ndd, true);
    $option_thvp_ndd = array();

    if (count($data_thvp_ndd) != 0) {
        foreach ($original_thvp_ndd as $key => $value_thvp_ndd) {
            if ($value_thvp_ndd["ngayXP"] != null) {
                $ngayXP = explode("-", $value_thvp_ndd["ngayXP"]);
            ...
            $option_thvp_ndd[] = array(
                'STT' => $key + 1,
                "id_thvp" => $value_thvp_ndd['id'],
                "ten_doanhnghiep" => $value_thvp_ndd['TenDoanhNghiep'],
                "so_quyetdinh_xp" => $value_thvp_ndd['quyetDinhXP'],
                "ngay_xp" => $ngayXP_new,
                "donvi_xp" => $value_thvp_ndd['donViXP'],
                "hinhthuc_xp" => $value_thvp_ndd['hinhThucXP'],
                "noidung_xp" => $value_thvp_ndd['noiDungXP'],
                "sotien_xp" => $value_thvp_ndd['soTienXP'],
                "tailieu_dinhkem" => '',
            );
        }
        $final_thvp_ndd = json_encode($option_thvp_ndd);
    } else {
        $final_thvp_ndd = json_encode([]);
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                        ...
                                        <form class="form-horizontal" method="post" id="info_ctkt_form" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group row list-ctkt">
                                                ...
                                                <div class="col-md-2">
                                                    <?php
                                                    echo '<input class="form-control input-sm" 
                                                            type="text" name="tongsogieng" id="tongsogieng"' .
                                                        'value=' . '"' . $data_info_ctkt[$i_check]["tongSoGiengKT"] . '"' . '>'
                                                    ?>
                                                </div>
                                            </div>
                                            <div class="form-group row list-ctkt">
                                                ...
                                                <div class="col-md-8">
                                                    ...
                                                    echo '<input class="form-control input-sm" 
                                                        ...
                                                        'value=' . '"' . $data_info_ctkt[$i_check]["tongLuuLuongKT"] . '"' . '>'
                                                    ?>
                                                </div>
                                                <!-- <label style="margin-top: 7px" class="col-md-2 control-label">
                                                    <b>Đơn vị</b>
                ...
                <div class="main-content">
                    <div class="container-fluid">
                        <div class="page-title" style="text-align: center">
                            <h3>
                                <b>CÔNG TRÌNH KHAI THÁC SỬ DỤNG NƯỚC DƯỚI ĐẤT</b>
                            </h3>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                            ...
                                            <div class="col-md-12">
                                                <form class="form-horizontal">
                                                    ...
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-2">
                                                            <?php
                                                            echo '<input class="form-control input-sm" id="tongso-gieng"
                                                                type="text"' . 'value=' . '"' . $data_info_ctkt[$i_check]["tongSoGiengKT"] . '"' . '>'
                                                            ?>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Tổng lưu lượng khai thác</b>
                                                        ...
                                                        <div class="col-md-2">
                                                            ...
                                                            echo '<input class="form-control input-sm" id="tong-llkt"
                                                                type="text"' . 'value=' . '"' . $data_info_ctkt[$i_check]["tongLuuLuongKT"] . '"' . '>'
                                                            ?>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Đơn vị</b>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        <h4 class="card-title text-bold text-info">
                                            Thông tin điểm Khai thác sử dụng Nước dưới đất
                                        </h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="poi_ktsd_ndd" class="table table-sm table-hover table-bordered">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_poi_ktsd_ndd">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_poi_ktsd_ndd">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_poi_ktsd_ndd">
                                                        <i class="ti-export pdd-right-5"></i>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="gp_ndd" class="table table-sm table-hover table-bordered">
                                                    <!-- DOM after Process Data -->
                                                </table>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_gp_ndd">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_gp_ndd">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_gp_ndd">
                                                        <i class="ti-export pdd-right-5"></i>
                                                        <span>Xuất dữ liệu</span>
                                                    </a> -->
                                                </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="tiencq_ndd" class="table table-sm table-hover table-bordered">
                                                    <!-- DOM after Process Data -->
                                                </table>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_tiencq_ndd">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_tiencq_ndd">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_tiencq_ndd">
                                                        <i class="ti-export pdd-right-5"></i>
                                                        <span>Xuất dữ liệu</span>
                                                    </a> -->
                                                </div>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        <h4 class="card-title text-bold text-info">
                                            Tình hình vi phạm khai thác sử dụng Nước dưới đất của Chủ công trình
                                        </h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table id="thvp_ndd" class="table table-sm table-hover table-bordered">
                                                    <!-- DOM after Process Data -->
                                                </table>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-info btn-sm" id="create_thvp_ndd">
                                                        <i class="ti-plus pdd-right-5"></i>
                                                        <span>Thêm mới</span>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm" id="delete_thvp_ndd">
                                                        <i class="ti-eraser pdd-right-5"></i>
                                                        <span>Xóa</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-success btn-sm" id="export_thvp_ndd">
                                                        <i class="ti-export pdd-right-5"></i>
                                                        <span>Xuất dữ liệu</span>
                                                    </a> -->
                                                </div>
    ...
    <script>
        ...
        var columns_title = [{
            ...
            },
            {
                ...
                cellClick: function(e, row, formatterParams) {
                    const idgieng = row.getData().id_gieng;
                    window.open("../../map.php?idpoi=" + idgieng + "&type=ktsd_ndd")
                }
            },
            {
                title: "#",
            ...
            },
            {
                ...
                formatter: "link",
                formatterParams: {
                    labelField: "sohieugieng",
                    <?php
                    echo 'urlPrefix: "form-poi-ktsd-ndd.php?macongtrinh=' . $macongtrinh . '&idgieng=",'
                    ?>
                    target: "_blank",
                }
            },
            ...
            {
                title: "Tình trạng giếng",
                field: "tinhtranggieng",
                hozAlign: "center"
            },
            {
                title: "Vùng bảo hộ",
        ...
        /*---- Lấy JSON bằng Php ----*/
        <?php
        $script = 'var tabledata = ' . $final_poi_ktsd_ndd . ";";
        echo $script;
        ?>

        $("#poi_ktsd_ndd").css("border-top", "1px solid #dee2e6");
        var selected_id;
        var table = new Tabulator("#poi_ktsd_ndd", {
        ...
        /*** Delete DiemKTSD_NDD ***/
        $("#delete_poi_ktsd_ndd").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                ...
                if (confirm("Bạn chắc chắn muốn xóa các điểm khai thác đã chọn này ?")) {
                    ...
                    selected_id.forEach(function(item, index) {
                        /*** Get id_gieng ***/
                        id_gieng.push(item.id_gieng);
                    })
                    var response_post = $.post("delete-poi-ktsd-ndd.php", {
                        <?php
        ...
        /*** Create new DiemKTSD_NDD (Button) ***/
        $("#create_poi_ktsd_ndd").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-poi-ktsd-ndd.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        ...
        var columns_title = [{
            ...
            },
            {
                ...
                formatterParams: {
                    ...
                    <?php
                    echo 'urlPrefix: "form-gp-ndd.php?macongtrinh=' . $macongtrinh . '&idgp=",'
                    ?>
                    target: "_blank",
                }
            },
        ...
        /*---- Lấy JSON bằng Php ----*/
        <?php
        $script = 'var tabledata = ' . $final_gp_ndd . ";";
        echo $script;
        ?>
        $("#gp_ndd").css("border-top", "1px solid #dee2e6");
        var selected_id;
        var table = new Tabulator("#gp_ndd", {
            columns: columns_title,
            rowSelectionChanged: function(data, rows) {
                selected_id = data;
            }
        ...
        /*** Delete ThongtinCP_NDD ***/
        $("#delete_gp_ndd").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                ...
                if (confirm("Bạn chắc chắn muốn xóa các giấy phép đã chọn này ?")) {
                    ...
                    var response_post = $.post("delete-gp-ndd.php", {
                        <?php
                        echo '"macongtrinh":' . $macongtrinh . ",";
                        ?> "id_gp": id_gp
                    }).done(function(data) {
        ...
        /*** Create new ThongtinCP_NDD (Button) ***/
        $("#create_gp_ndd").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-gp-ndd.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        ...
        var columns_title = [{
            ...
            },
            {
                ...
                formatterParams: {
                    ...
                    <?php
                    echo 'urlPrefix: "form-tiencq-ndd.php?macongtrinh=' . $macongtrinh . '&idtiencq=",'
                    ?>
                    target: "_blank",
                }
            },
        ...
        /*---- Lấy JSON bằng Php ----*/
        <?php
        $script = 'var tabledata = ' . $final_tiencq_ndd . ";";
        echo $script;
        ?>
        <?php
        if (count($data_tiencq_ndd) != 0) {
            echo '$("#tiencq_ndd").css("border-top", "1px solid #dee2e6");';
        } else {
            echo '$("#tiencq_ndd").css("border-top", "none");';
        }
        ?>
        var selected_id;
        var table = new Tabulator("#tiencq_ndd", {
            <?php
            if (count($data_tiencq_ndd) != 0) {
        ...
        /*** Delete TienCapQuyen_NDD ***/
        $("#delete_tiencq_ndd").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                ...
                if (confirm("Bạn chắc chắn muốn xóa các mục tiền cấp quyền đã chọn này ?")) {
                    ...
                    var response_post = $.post("delete-tiencq-ndd.php", {
                        "id_tiencq": id_tiencq
                    }).done(function(data) {
                        if (data != "error") {
                            location.reload();
        ...
        /*** Create new TienCapQuyen_NDD (Button) ***/
        $("#create_tiencq_ndd").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-tiencq-ndd.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        var columns_title = [{
            ...
            },
            {
                ...
                formatterParams: {
                    ...
                    <?php
                    echo 'urlPrefix: "form-thvp-ndd.php?macongtrinh=' . $macongtrinh . '&idthvp=",'
                    ?>
                    target: "_blank",
                }
            },
        ...
        <?php
        if (isset($data_gp_ndd[0]["ma_doanhnghiep"])) {
            $script = 'var tabledata = ' . $final_thvp_ndd . ";";
            echo $script;
        }
        ?>
        <?php
        if (isset($data_gp_ndd[0]["ma_doanhnghiep"])) {
            if (count($data_thvp_ndd) != 0) {
                echo '$("#thvp_ndd").css("border-top", "1px solid #dee2e6");';
            } else {
                echo '$("#thvp_ndd").css("border-top", "none");';
            }
        }
        ?>
        var selected_id;
        var table = new Tabulator("#thvp_ndd", {
            <?php
            if (isset($data_gp_ndd[0]["ma_doanhnghiep"])) {
                if (count($data_thvp_ndd) != 0) {
                    echo 'columns: columns_title,
        ...
        /*** Delete TinhHinhViPham_NDD ***/
        $("#delete_thvp_ndd").click(function(e) {
            e.preventDefault();
            if (selected_id.length == 0) {
                alert("Vui lòng chọn giấy phép cần xóa")
            } else {
                if (confirm("Bạn chắc chắn muốn xóa các giấy phép đã chọn này ?")) {
                    ...
                    var response_post = $.post("delete-thvp-ndd.php", {
                        <?php
                        echo '"macongtrinh":' . $macongtrinh . ",";
                        ?> "id_thvp": id_thvp
                    }).done(function(data) {
        ...
        /*** Create new TinhHinhViPham_NDD (Button) ***/
        $("#create_thvp_ndd").click(function(e) {
            e.preventDefault();
            <?php
            echo 'location.href = "form-thvp-ndd.php?macongtrinh=' . $macongtrinh . '"';
            ?>
        })
    </script>
    ...
    <script>
        ...
        $("#update_info_ctkt").click(function() {
            ...
            var response_post = $.post("update-ndd-info.php", {
                <?php
                echo '"macongtrinh":' . $macongtrinh . ",";
                ?>
                /*** CT_KTSD ***/
                ...
                "tongsogieng": tongsogieng == "" ? 'NULL' : tongsogieng,
