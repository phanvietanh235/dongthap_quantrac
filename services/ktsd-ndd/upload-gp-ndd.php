...
<?php
    ...
    mkdir($targetfolder);
    $targetfolder = $targetfolder . basename($_FILES['uploaded_qp_ndd']['name']);
    $file_type = $_FILES['uploaded_qp_ndd']['type'];
    if ($file_type == "application/pdf") {
        if (move_uploaded_file($_FILES['uploaded_qp_ndd']['tmp_name'], $targetfolder)) {
            echo "success";
        } else {
            echo "error";
        }
