...
if (isset($_GET['ma'])) {
    ...
    $result_location = pg_query($tiengiang_db, $query_location);
    if (!$result_location) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_location = array();
    while ($row = pg_fetch_assoc($result_location)) {
        $data_location[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_location" 
                                                                        value=' . '"' . $data_location[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_location"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-4">
                                                            <select class="input-sm form-control" id="type_location">
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-10">
                                                            ...
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="note">' .
                                                                    $data_location[0]["note"] . '</textarea>';
                                                            } else {
                                                                echo '<textarea class="form-control" id="note"></textarea>';
                                                            }
                                                            ?>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_location">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/diadanh.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        ...
        $.getJSON("../locationType.php", function(type_location) {
            /* $('#type_location')
                .append($("<option></option>")
                    .attr('value', 'none').text("--Loại doanh nghiệp--")); */
            $.each(type_location, function(key, value) {
                /*** Đánh dấu Option Loại doanh nghiệp được chọn ***/
                <?php
                if (isset($_GET['ma'])) {
                    echo 'if (value.id == ' . $data_location[0]["locationtypeid"] . ') {
                            $("#type_location")
                                .append($("<option></option>")
    ...
    <script>
        $("#save_location").click(function() {
            var ten_location = $("#ten_location").val();
            var type_location = $("#type_location").val();
            var note = $("#note").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-location.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Địa danh ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
