...
<?php
if (isset($_GET['ma'])) {
    $query_type_enter = 'SELECT * FROM "LoaiHinhDoanhNghiep"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_type_enter = pg_query($tiengiang_db, $query_type_enter);
    if (!$result_type_enter) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_type_enter = array();
    while ($row = pg_fetch_assoc($result_type_enter)) {
        $data_type_enter[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="cancu_phaply" 
                                                                        value=' . '"' . $data_type_enter[0]["canCuPhapLy"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="cancu_phaply"/>';
                                                            }
                                                            ?>
                                                    ...
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-10">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="mota">' .
                                                                    $data_type_enter[0]["moTa"] . '</textarea>';
                                                            } else {
                                                                echo '<textarea class="form-control" id="mota"></textarea>';
                                                            }
                                                            ?>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_type_enter">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/loaihinhdoanhnghiep.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_type_enter").click(function() {
            var ten_loaihinh = $("#ten_loaihinh").val();
            var cancu_phaply = $("#cancu_phaply").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-type-enter.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Doanh nghiệp ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
