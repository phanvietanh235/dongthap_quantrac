...
<?php
if (isset($_GET['ma'])) {
    $query_mdsd = 'SELECT * FROM "Purpose"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_mdsd = pg_query($tiengiang_db, $query_mdsd);
    if (!$result_mdsd) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_mdsd = array();
    while ($row = pg_fetch_assoc($result_mdsd)) {
        $data_mdsd[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_mdsd" 
                                                                            value=' . '"' . $data_mdsd[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_mdsd"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-10">
                                                            ...
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="mota">' .
                                                                    $data_mdsd[0]["description"] . '</textarea>';
                                                            } else {
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_mdsd">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/mucdichsudung.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_mdsd").click(function() {
            var ten_mdsd = $("#ten_mdsd").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-mdsd.php", {
                <?php
                if (isset($_GET['ma'])) {
