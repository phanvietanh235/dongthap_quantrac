...
<?php
if (isset($_GET['ma'])) {
    $query_dt_ktsd = 'SELECT * FROM "DoiTuongKTSD"
                        WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_dt_ktsd = pg_query($tiengiang_db, $query_dt_ktsd);
    if (!$result_dt_ktsd) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_dt_ktsd = array();
    while ($row = pg_fetch_assoc($result_dt_ktsd)) {
        $data_dt_ktsd[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        <h4 class="card-title text-bold">
                                            THÔNG TIN ĐỐI TƯỢNG KHAI THÁC SỬ DỤNG
                                        </h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_dt_ktsd" 
                                                                        value=' . '"' . $data_dt_ktsd[0]["tenDoiTuongKTSD"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_dt_ktsd"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_dt_ktsd">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/doituongktsd.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_dt_ktsd").click(function() {
            var ten_dt_ktsd = $("#ten_dt_ktsd").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-dt-ktsd.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đối tượng KTSD ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_dt_ktsd": ten_dt_ktsd,
                "mota": mota,
            }).done(function(data) {
                if (data == "error") {
