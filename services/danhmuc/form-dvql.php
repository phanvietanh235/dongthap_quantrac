...
<?php
if (isset($_GET['ma'])) {
    $query_dvql = 'SELECT * FROM "DonViQuanLy"
                                WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_dvql = pg_query($tiengiang_db, $query_dvql);
    if (!$result_dvql) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_dvql = array();
    while ($row = pg_fetch_assoc($result_dvql)) {
        $data_dvql[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                        ...
                                                        <div class="col-md-10">
                                                            ...
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="mota">' .
                                                                    $data_dvql[0]["description"] . '</textarea>';
                                                            } else {
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_dvql">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/donviquanly.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_dvql").click(function() {
            var ten_donvi = $("#ten_donvi").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-dvql.php", {
                <?php
                if (isset($_GET['ma'])) {
