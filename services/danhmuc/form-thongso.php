...
<?php
if (isset($_GET['ma'])) {
    $query_thongso = 'SELECT * FROM "Parameter"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_thongso = pg_query($tiengiang_db, $query_thongso);
    if (!$result_thongso) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_thongso = array();
    while ($row = pg_fetch_assoc($result_thongso)) {
        $data_thongso[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_thongso" 
                                                                            value=' . '"' . $data_thongso[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_thongso"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="code_thongso" 
                                                                            value=' . '"' . $data_thongso[0]["code"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="code_thongso"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_thongso">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/thongso.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_thongso").click(function() {
            var ten_thongso = $("#ten_thongso").val();
            var code_thongso = $("#code_thongso").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-thongso.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đơn vị cấp phép ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_thongso": ten_thongso,
                "code_thongso": code_thongso,
            }).done(function(data) {
                if (data == "error") {
