...
<?php
if (isset($_GET['ma'])) {
    $query_loaigp = 'SELECT * FROM "LoaiGiayPhep"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_loaigp = pg_query($tiengiang_db, $query_loaigp);
    if (!$result_loaigp) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_loaigp = array();
    while ($row = pg_fetch_assoc($result_loaigp)) {
        $data_loaigp[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaigp" 
                                                                            value=' . '"' . $data_loaigp[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaigp"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_loaigp">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/loaigiayphep.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_loaigp").click(function() {
            var ten_loaigp = $("#ten_loaigp").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-loaigp.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đơn vị cấp phép ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_loaigp": ten_loaigp,
            }).done(function(data) {
                if (data == "error") {
