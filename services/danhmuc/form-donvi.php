...
<?php
if (isset($_GET['ma'])) {
    $query_donvi = 'SELECT * FROM "Unit"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_donvi = pg_query($tiengiang_db, $query_donvi);
    if (!$result_donvi) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_donvi = array();
    while ($row = pg_fetch_assoc($result_donvi)) {
        $data_donvi[] = $row;
    }
}
?>
                                            ...
                                            <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                <div class="form-group row list-ctkt">
                                                    ...
                                                    <div class="col-md-4">
                                                        <?php
                                                        if (isset($_GET['ma'])) {
                                                            echo '<input type="text" class="form-control input-sm" id="code_donvi" 
                                                                            value=' . '"' . $data_donvi[0]["code"] . '"' . '/>';
                                                        } else {
                                                            echo '<input type="text" class="form-control input-sm" id="code_donvi"/>';
                                                        }
                                                        ?>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="list-ctkt text-right">
                                                <a href="#" class="btn btn-success btn-sm" id="save_donvi">
                                                    <i class="ti-save-alt pdd-right-5"></i>
                                                    <span>Lưu thông tin</span>
                                                </a>
                                                <a href="../../danhmuc/donvi.php" class="btn btn-info btn-sm">
                                                    <i class="ti-save-alt pdd-right-5"></i>
                                                    <span>Quay lại</span>
                                                </a>
                                            </div>
...
<script>
    $("#save_donvi").click(function() {
        var ten_donvi = $("#ten_donvi").val();
        var code_donvi = $("#code_donvi").val();

        /*---- Update CSDL ----*/
        var response_post = $.post("save-donvi.php", {
            <?php
            if (isset($_GET['ma'])) {
