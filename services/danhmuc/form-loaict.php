...
<?php
if (isset($_GET['ma'])) {
    $query_loaict = 'SELECT * FROM "LoaiCongTrinh"
                                WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_loaict = pg_query($tiengiang_db, $query_loaict);
    if (!$result_loaict) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_loaict = array();
    while ($row = pg_fetch_assoc($result_loaict)) {
        $data_loaict[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Tên loại công trình</b>
                                                        </label>
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaict" 
                                                                        value=' . '"' . $data_loaict[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaict"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="code_loaict" 
                                                                        value=' . '"' . $data_loaict[0]["type_ct"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="code_loaict"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_loaict">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/loaicongtrinh.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_loaict").click(function() {
            var ten_loaict = $("#ten_loaict").val();
            var code_loaict = $("#code_loaict").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-loaict.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đơn vị cấp phép ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_loaict": ten_loaict,
                "code_loaict": code_loaict,
            }).done(function(data) {
                if (data == "error") {
