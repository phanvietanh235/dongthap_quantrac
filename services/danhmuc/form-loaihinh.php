...
<?php
if (isset($_GET['ma'])) {
    $query_loaihinh = 'SELECT * FROM "ObservationType"
                                WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_loaihinh = pg_query($tiengiang_db, $query_loaihinh);
    if (!$result_loaihinh) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_loaihinh = array();
    while ($row = pg_fetch_assoc($result_loaihinh)) {
        $data_loaihinh[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-2">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaihinh" 
                                                                        value=' . '"' . $data_loaihinh[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaihinh"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Loại hình tổng quát</b>
                                                        </label>
                                                        <div class="col-md-2">
                                                            <select class="form-control input-sm" id="type_loaihinh">
                                                                <option value='0'></option>
                                                                <option value="1">Không khí</option>
                                                                <option value="2">Nước mặt</option>
                                                                <option value="3">Nước ngầm</option>
                                                                <option value="4">Nước thải</option>
                                                            </select>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Mã loại hình</b>
                                                        ...
                                                        <div class="col-md-2">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="code" 
                                                                        value=' . '"' . $data_loaihinh[0]["code"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="code"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_loaihinh">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/loaihinh.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        <?php
        if (isset($_GET['ma']) && $_GET['parent'] != null) {
            echo '$("#type_loaihinh").val(' . $_GET['parent'] . ')';
        }
        ?>
    </script>

    <script>
        $("#save_loaihinh").click(function() {
            var ten_loaihinh = $("#ten_loaihinh").val();
            var type_loaihinh = $("#type_loaihinh").val();
            var code = $("#code").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-loaihinh.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đơn vị cấp phép ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_loaihinh": ten_loaihinh,
                "type_loaihinh": type_loaihinh != 0 ? type_loaihinh : null,
                "code": code,
            }).done(function(data) {
                if (data == "error") {
