...
<?php
if (isset($_GET['ma'])) {
    $query_dvcp = 'SELECT * FROM "DonViCapPhep"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_dvcp = pg_query($tiengiang_db, $query_dvcp);
    if (!$result_dvcp) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_dvcp = array();
    while ($row = pg_fetch_assoc($result_dvcp)) {
        $data_dvcp[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_donvi" 
                                                                            value=' . '"' . $data_dvcp[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_donvi"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Loại đơn vị cấp phép</b>
                                                        </label>
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="type_dvcp" 
                                                                            value=' . '"' . $data_dvcp[0]["type_dvcp"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="type_dvcp"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-10">
                                                            ...
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="mota">' .
                                                                    $data_dvcp[0]["description"] . '</textarea>';
                                                            } else {
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_dvcp">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/donvicapphep.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_dvcp").click(function() {
            var ten_donvi = $("#ten_donvi").val();
            var type_dvcp = $("#type_dvcp").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-dvcp.php", {
                <?php
                if (isset($_GET['ma'])) {
