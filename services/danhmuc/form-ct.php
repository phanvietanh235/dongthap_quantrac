...
<?php
if (isset($_GET['ma'])) {
    $query_ct = 'SELECT * FROM "StandardParameter"
                            WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_ct = pg_query($tiengiang_db, $query_ct);
    if (!$result_ct) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_ct = array();
    while ($row = pg_fetch_assoc($result_ct)) {
        $data_ct[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <select class="input-sm form-control" id="standard">
                                                            </select>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Mục đích</b>
                                                        ...
                                                        <div class="col-md-4">
                                                            <select class="input-sm form-control" id="purpose">
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <select class="input-sm form-control" id="para">
                                                            </select>
                                                        </div>
                                                        <label style="margin-top: 7px" class="col-md-2 control-label">
                                                            <b>Đơn vị</b>
                                                        ...
                                                        <div class="col-md-4">
                                                            <select class="input-sm form-control" id="unit">
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="max_ct" 
                                                                            value=' . '"' . $data_ct[0]["maxvalue"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="max_ct"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="min_ct" 
                                                                            value=' . '"' . $data_ct[0]["minvalue"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="min_ct"/>';
                                                            }
                                                            ?>
                                                    ...
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-10">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<textarea class="form-control" id="method">' .
                                                                    $data_ct[0]["analysismethod"] . '</textarea>';
                                                            } else {
                                                                echo '<textarea class="form-control" id="method"></textarea>';
                                                            }
                                                            ?>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_ct">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/chitieu.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        ...
        $.getJSON("../standard-option.php", function(standard) {
            /* $('#standard')
                .append($("<option></option>")
                    .attr('value', 'none').text("--Loại quy chuẩn--")); */
            $.each(standard, function(key, value) {
                /*** Đánh dấu Option Loại Quy chuẩn được chọn ***/
                <?php
                if (isset($_GET['ma']) && $data_ct[0]["standardid"] != null) {
                    echo 'if (value.id == ' . $data_ct[0]["standardid"] . ') {
                            $("#standard")
                                .append($("<option></option>")
        ...
        /*** DOM Option Thông số ***/
        $.getJSON("../para-option.php", function(para) {
            /* $('#para')
                .append($("<option></option>")
                    .attr('value', 'none').text("--Loại thông số--")); */
            $.each(para, function(key, value) {
                /*** Đánh dấu Option Loại Thông số được chọn ***/
                <?php
                if (isset($_GET['ma']) && $data_ct[0]["parameterid"] != null) {
                    echo 'if (value.id == ' . $data_ct[0]["parameterid"] . ') {
                            $("#para")
                                .append($("<option></option>")
                                    .attr("value", value.id).attr("selected", "selected").text(value.name));
                            } else {
        ...
        /*** DOM Option Đơn vị ***/
        $.getJSON("../unit-option.php", function(unit) {
            /* $('#para')
                .append($("<option></option>")
                    .attr('value', 'none').text("--Loại đơn vị--")); */
            $.each(unit, function(key, value) {
                /*** Đánh dấu Option Loại Đơn vị được chọn ***/
                <?php
                if (isset($_GET['ma']) && $data_ct[0]["unitid"] != null) {
                    echo 'if (value.id == ' . $data_ct[0]["unitid"] . ') {
                            $("#unit")
                                .append($("<option></option>")
                                    .attr("value", value.id).attr("selected", "selected").text(value.name));
                            } else {
        ...
        /*** DOM Option Mục đích ***/
        $.getJSON("../purpose-option.php", function(purpose) {
            $('#purpose')
                .append($("<option></option>")
                    .attr('value', 'none').text("--Loại mục đích--"));
            $.each(purpose, function(key, value) {
                /*** Đánh dấu Option Loại Mục đích được chọn ***/
                <?php
                if (isset($_GET['ma']) && $data_ct[0]["purposeid"]) {
                    echo 'if (value.id == ' . $data_ct[0]["purposeid"] . ') {
                            $("#purpose")
                                .append($("<option></option>")
                                    .attr("value", value.id).attr("selected", "selected").text(value.name));
                            } else {
    ...
    <script>
        $("#save_ct").click(function() {
            ...
            var para = $("#para").val();
            var unit = $("#unit").val();
            var min_ct = $("#min_ct").val();
            var max_ct = $("#max_ct").val();
            var method = $("#method").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-ct.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Chỉ tiêu ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
