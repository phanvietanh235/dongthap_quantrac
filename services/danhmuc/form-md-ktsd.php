...
<?php
if (isset($_GET['ma'])) {
    $query_md_ktsd = 'SELECT * FROM "MucDichKTSD"
                        WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_md_ktsd = pg_query($tiengiang_db, $query_md_ktsd);
    if (!$result_md_ktsd) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_md_ktsd = array();
    while ($row = pg_fetch_assoc($result_md_ktsd)) {
        $data_md_ktsd[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                ...
                                <div class="card">
                                    <div class="card-block pdd-vertical-15 pdd-horizon-15">
                                        <h4 class="card-title text-bold">
                                            THÔNG TIN MỤC ĐÍCH KHAI THÁC SỬ DỤNG
                                        </h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_md_ktsd" 
                                                                        value=' . '"' . $data_md_ktsd[0]["mucDich"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_md_ktsd"/>';
                                                            }
                                                            ?>
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="type_md_ktsd" 
                                                                        value=' . '"' . $data_md_ktsd[0]["type_mucdich"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="type_md_ktsd"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_md_ktsd">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/mucdichktsd.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_md_ktsd").click(function() {
            var ten_md_ktsd = $("#ten_md_ktsd").val();
            var type_md_ktsd = $("#type_md_ktsd").val();
            var mota = $("#mota").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-md-ktsd.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Mục đích KTSD ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_md_ktsd": ten_md_ktsd,
                "type_md_ktsd": type_md_ktsd,
                "mota": mota,
            }).done(function(data) {
                if (data == "error") {
