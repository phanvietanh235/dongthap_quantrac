...
<?php
if (isset($_GET['ma'])) {
    $query_loaitram = 'SELECT * FROM "Category"
                        WHERE id = ' . "'" . $_GET['ma'] . "'";

    $result_loaitram = pg_query($tiengiang_db, $query_loaitram);
    if (!$result_loaitram) {
        echo "Không có dữ liệu.\n";
        exit;
    }
    $data_loaitram = array();
    while ($row = pg_fetch_assoc($result_loaitram)) {
        $data_loaitram[] = $row;
    }
}
?>
...
<a href="../../HDSD_QuanTracDongThap.docx">Hướng dẫn sử dụng</a>
                                                ...
                                                <form class="form-horizontal" id="fileinfo" name="fileinfo" enctype="multipart/form-data">
                                                    <div class="form-group row list-ctkt">
                                                        ...
                                                        <div class="col-md-4">
                                                            <?php
                                                            if (isset($_GET['ma'])) {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaitram" 
                                                                        value=' . '"' . $data_loaitram[0]["name"] . '"' . '/>';
                                                            } else {
                                                                echo '<input type="text" class="form-control input-sm" id="ten_loaitram"/>';
                                                            }
                                                            ?>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="list-ctkt text-right">
                                                    <a href="#" class="btn btn-success btn-sm" id="save_loaitram">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Lưu thông tin</span>
                                                    </a>
                                                    <a href="../../danhmuc/loaitram.php" class="btn btn-info btn-sm">
                                                        <i class="ti-save-alt pdd-right-5"></i>
                                                        <span>Quay lại</span>
                                                    </a>
                                                </div>
    ...
    <script>
        $("#save_loaitram").click(function() {
            var ten_loaitram = $("#ten_loaitram").val();

            /*---- Update CSDL ----*/
            var response_post = $.post("save-loaitram.php", {
                <?php
                if (isset($_GET['ma'])) {
                    /*** Update Đơn vị cấp phép ***/
                    echo 'ma:' . $_GET['ma'] . ', ';
                }
                ?> "ten_loaitram": ten_loaitram,
            }).done(function(data) {
                if (data == "error") {
