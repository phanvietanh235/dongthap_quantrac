...
$ma_tramcu = $_POST["ma_tramcu"];

/*---- Thông tin Danh sách công trình KTSD NDD ----*/
$querry_data = 'SELECT * FROM list_diemqt_nm ';
$querry_data .= 'WHERE ';
...
//     }
// }
$querry_data .= '("ma_tramcu"::text IS NULL OR "ma_tramcu"::text ILIKE ' . "'%" . $ma_tramcu . "%'" . ') AND ';
$querry_data .= '("Sohieu" IS NULL OR "Sohieu" ILIKE ' . "'%" . $sohieu . "%'" . ') AND ';
/* $querry_data .= '("tendiadanh" IS NULL OR "tendiadanh" ILIKE ' . "'%" . $tendiadanh . "%'" . ') AND '; */
...
foreach ($original_data as $key => $value) {
    $option[] = array(
        ...
        "ma_tramcu" => $value['ma_tramcu'],
        "tendiadanh" => $value['tendiadanh'],
