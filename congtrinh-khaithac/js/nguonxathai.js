...
function search_list_ctkt_xt() {
    ...
    var columns_title = [
        ...
        {title: "Tên nguồn thải", field: "tencongtrinh", frozen: true, formatter: "textarea", width: 350},
        {title: "Địa chỉ nguồn thải", field: "diachi_ct", formatter: "textarea", width: 250},
        {title: "Quận/huyện", field: "quanhuyen", hozAlign: "center", width: 150},
        {title: "Phường/xã", field: "phuongxa", hozAlign: "center", width: 150},
        {title: "Tên doanh nghiệp (Tên chủ giấy phép)", field: "doanhnghiep", formatter: "textarea", width: 250},
        {title: "Địa chỉ doanh nghiệp", field: "diachidoanhnghiep", formatter: "textarea", width: 200},
        {title: "Cơ sở sản xuất", field: "coso_sanxuat", formatter: "textarea", width: 200},
        {title: "Số giấy phép mới nhất", field: "sogiayphep", hozAlign: "center"},
        {title: "Năm xây dựng", field: "nam_xd", hozAlign: "center"},
    ]

    /*** POST send Data ***/
    var response_post = $.post("../services/ktsd-xt/ktsd-xathai.php", {
    ...
    response_post.done(function (data) {
        if (data.length != 0) {
            $("#list_ctkt_xt_table").css("border-top", "1px solid #dee2e6");
            table = new Tabulator("#list_ctkt_xt_table", {
                selectable: 1,
                rowClick: function (e, row) {
                    var index = row._row.data.macongtrinh;
                    window.open("../services/ktsd-xt/ktsd-xt-info.php?macongtrinh=" + index, '_blank');
                    /* location.href = "../services/ktsd-xt-info.php?macongtrinh=" + index; */
                },
                ajaxURL: "../services/ktsd-xt/ktsd-xathai.php",
                ajaxParams: {
                    "tencongtrinh": tencongtrinh,
                    "quanhuyen": quanhuyen,
                    "doanhnghiep": doanhnghiep,
            ...
            /* $("#list_ctkt_xt_table").css("border-top", "none");
            table = new Tabulator("#list_ctkt_xt_table", {
                height: "70px",
                placeholder: "<p class='text-danger text-bold font-size-14'>" +
                    "<i class='icon-alert pdd-right-5'></i>Không có dữ liệu!</p>",
...
$("#search_list_ctkt_xt").click(function () {
    search_list_ctkt_xt();
...
$("#export_list_ctkt_xt").click(function () {
    var tencongtrinh = $("#name_ct").val();
    var quanhuyen = $("#quanhuyen").val();
    var doanhnghiep = $("#doanhnghiep").val();
    var sogiayphep = $("#sogiayphep").val();
    ...
    response_post.done(function (data) {
        if (data.length != 0) {
            ...
            XLSX.writeFile(wb, 'ktsd_nguonxathai.xlsx')
        }
    })
})
...
$("#add_info_ctkt").on("click", function () {
    ...
    if (ma_dvhc == 'none' || tencongtrinh == '' || diachi_congtrinh == '') {
        ...
        var response_post = $.post("../services/ktsd-xt/add-xt-info.php", {
            /*** CT_KTSD ***/
            "tencongtrinh": tencongtrinh,
            ...
            if (data != "error") {
                ...
                window.open("../services/ktsd-xt/ktsd-xt-info.php?macongtrinh=" + data, '_blank');
            } else {
