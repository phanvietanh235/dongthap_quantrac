...
function search_list_ctkt_nm() {
    ...
    var columns_title = [
        ...
        {title: "Số giấy phép mới nhất", field: "sogiayphep", hozAlign: "center", width: 200},
        {title: "Năm xây dựng", field: "nam_xd", hozAlign: "center", width: 150},
    ]

    /*** POST send Data ***/
    var response_post = $.post("../services/ktsd-nm/ktsd-nuocmat.php", {
    ...
    response_post.done(function (data) {
        if (data.length != 0) {
            $("#list_ctkt_nm_table").css("border-top", "1px solid #dee2e6");
            table = new Tabulator("#list_ctkt_nm_table", {
                selectable: 1,
                rowClick: function (e, row) {
                    var index = row._row.data.macongtrinh;
                    window.open("../services/ktsd-nm/ktsd-nm-info.php?macongtrinh=" + index, '_blank');
                    /* location.href = "../services/ktsd-nm-info.php?macongtrinh=" + index; */
                },
                ajaxURL: "../services/ktsd-nm/ktsd-nuocmat.php",
                ajaxParams: {
                    "tencongtrinh": tencongtrinh,
                    "quanhuyen": quanhuyen,
                    "doanhnghiep": doanhnghiep,
...
$("#search_list_ctkt_nm").click(function () {
    search_list_ctkt_nm();
...
$("#export_list_ctkt_nm").click(function () {
    var tencongtrinh = $("#name_ct").val();
    var quanhuyen = $("#quanhuyen").val();
    var doanhnghiep = $("#doanhnghiep").val();
    var sogiayphep = $("#sogiayphep").val();
    ...
    response_post.done(function (data) {
        if (data.length != 0) {
            ...
            XLSX.writeFile(wb, 'ktsd_nuocmat.xlsx')
        }
    })
})
...
$("#add_info_ctkt").on("click", function () {
    ...
    if (ma_dvhc == 'none' || tencongtrinh == '' || diachi_congtrinh == '') {
        ...
        var response_post = $.post("../services/ktsd-nm/add-nm-info.php", {
            /*** CT_KTSD ***/
            "tencongtrinh": tencongtrinh,
            ...
            if (data != "error") {
                ...
                window.open("../services/ktsd-nm/ktsd-nm-info.php?macongtrinh=" + data, '_blank');
            } else {
